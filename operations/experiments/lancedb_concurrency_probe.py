...
class Metrics:
    ...
    writes_ok: int = 0
    writes_err: int = 0
    # Number of search iterations that returned
    # at least one newly inserted row (id > seed_rows)
...
def writer_worker(
    ...
    while time.time() < end_time:
        try:
            ...
            # 2) Insert brand new rows with embeddings
            # (simulates new chunks being indexed)
            new_rows = []
