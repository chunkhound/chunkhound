...
class AutodocNoPointsError(RuntimeError):
    """Raised when autodoc overview produces no points of interest."""

    def __init__(self, overview_answer: str) -> None:
        super().__init__("Autodoc overview produced no points of interest.")
        self.overview_answer = overview_answer


@dataclass
class AutodocPipelineResult:
...
async def run_autodoc_overview_only(
    *,
    ...
    """Run overview-only autodoc and return the answer + points."""
    ...
    if not points_of_interest:
        raise AutodocNoPointsError(overview_answer)

    return overview_answer, points_of_interest


async def run_autodoc_pipeline(
    *,
    services: Any,
...
) -> AutodocPipelineResult:
    """Run autodoc overview + per-point deep research and compute coverage."""
    overview_answer, points_of_interest = await _run_autodoc_overview_hyde(
        llm_manager=llm_manager,
        target_dir=target_dir,
        scope_path=scope_path,
    ...
    for idx, poi in enumerate(points_of_interest, start=1):
        ...
        try:
            ...
            if log_error:
                log_error(f"Autodoc deep research failed for point {idx}: {exc}")
            logger.exception(f"Autodoc deep research failed for point {idx}.")
