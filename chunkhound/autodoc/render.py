...
from chunkhound.autodoc.pipeline import _derive_heading_from_point, _slugify_heading
...
def render_overview_document(
    *,
    ...
    """Render overview-only autodoc output."""
    lines: list[str] = [
        format_metadata_block(meta).rstrip("\n"),
        f"# AutoDoc Overview for {scope_label}",
        "",
...
def render_combined_document(
    *,
    ...
    """Render the combined autodoc document for a scope."""
    lines: list[str] = [
        format_metadata_block(meta).rstrip("\n"),
        f"# AutoDoc for {scope_label}",
        "",
    ]
...
def render_index_document(
    *,
    ...
    """Render the per-scope index of autodoc topics."""
    lines: list[str] = [
        format_metadata_block(meta).rstrip("\n"),
        f"# AutoDoc Topics for {scope_label}",
        "",
