"""Depth Exploration Service for Coverage-First Research (Phase 1.5).

This module implements Phase 1.5 of the v2 coverage-first research algorithm:
...
class DepthExplorationService:
    ...
    already in coverage. This mimics V1's follow-up question behavior
    while maintaining V2's coverage-first philosophy.
    """
    ...
    async def _generate_all_exploration_queries(
        ...
        for result in results:
            if isinstance(result, Exception):
                logger.warning(f"Query generation failed: {result}")
                continue
            file_path, queries = result
            exploration_queries[file_path] = queries

        return exploration_queries
    ...
    async def _generate_exploration_queries(
        ...
        # Build chunk summary
        chunk_summaries = []
        for chunk in file_chunks[:5]:  # Limit to top 5 chunks for context
