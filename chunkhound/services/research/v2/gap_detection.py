"""Gap Detection Service for Coverage-First Research (Phase 2).

This module implements Phase 2 of the v2 coverage-first research algorithm:
1. Cluster covered chunks using HDBSCAN
...
class GapDetectionService:
    ...
    async def detect_and_fill_gaps(
        ...
        # Step 2.1: Cluster chunks with HDBSCAN
        cluster_groups = await self._cluster_chunks_hdbscan(covered_chunks)
    ...
    async def _cluster_chunks_hdbscan(self, chunks: list[dict]) -> list[list[dict]]:
        """Step 2.1: Cluster chunks using HDBSCAN for semantic grouping.
        ...
        try:
            ...
            embeddings_list = await embedding_provider.embed(chunk_contents)
        ...
        # HDBSCAN clustering (same pattern as clustering_service.py)
        min_cluster_size = min(self._config.min_cluster_size, len(embeddings_array) - 1)
        ...
        logger.debug(
            f"HDBSCAN: {len(chunks)} chunks → {len(cluster_groups)} clusters "
            f"(outliers: {sum(1 for label in labels if label == -1)})"
        )
    ...
    async def _detect_gaps_parallel(
        ...
        async def detect_gaps_in_shard(
            ...
            async with semaphore:
                # Extract imports per file for header injection
                file_imports: dict[str, list[str]] = {}
                if self._import_context_service:
                    for chunk in shard:
                        file_path = chunk.get("file_path", "")
                        if file_path and file_path not in file_imports:
                ...
                # Build code context with imports header injection
                code_lines = []
                for chunk in shard:
                    file_path = chunk.get("file_path", "unknown")
                    code = chunk.get("content", "")
                    # Prepend imports header if available
                    imports_header = ""
