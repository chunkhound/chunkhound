"""Test to reproduce the oversized chunk error (325k+ tokens).

This test reproduces the exact error:
"Failed to generate embeddings: Error code: 400 -
{'error': {'message': 'Requested 325998 tokens, max 300000 tokens per request'"
"""
...
from .fixtures.fake_providers import FakeEmbeddingProvider
...
def very_long_function_that_might_not_be_chunked_properly():
    ...
    data = {
        ...
        assert len(full_content) == target_chars, (
            f"Expected {target_chars} chars, got {len(full_content)}"
        )
        return full_content
    ...
    @pytest.mark.asyncio
    async def test_reproduce_with_mock_provider_for_debugging(
        self, services, temp_dir, test_config
    ):
        """Reproduce the chunking behavior without hitting OpenAI API."""
        ...
        # Create embedding provider that simulates token limit errors
        class TokenLimitEmbeddingProvider(FakeEmbeddingProvider):
            """FakeEmbeddingProvider that raises BadRequestError for oversized."""
            ...
            async def embed(self, texts: list[str]) -> list[list[float]]:
                ...
                print("Mock provider called with:")
                ...
                for i, text in enumerate(texts):
                    if len(text) > 200000:
                        print(f"  Warning: Chunk {i}: {len(text):,} characters")
                ...
                # Return hash-based unique embeddings via parent class
                return await super().embed(texts)

        mock_provider = TokenLimitEmbeddingProvider(model="mock-openai", dims=1536)
    ...
    @pytest.mark.asyncio
    async def test_analyze_chunking_behavior_with_large_files(
        self, services, temp_dir, test_config
    ):
        ...
        for name, size, description in test_cases:
            ...
            # Create FakeEmbeddingProvider with hash-based unique vectors
            mock_provider = FakeEmbeddingProvider(model="analysis-mock", dims=1536)
            ...
            if chunk_count > 0:
                ...
                if oversized:
                    print(f"  Warning: Oversized chunks: {len(oversized)}")
                    print(f"  Sizes: {oversized}")
                else:
                    print("  ✅ All chunks under 100k chars")
            else:
