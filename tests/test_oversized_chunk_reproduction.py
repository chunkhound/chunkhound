"""
Test to reproduce the oversized chunk error

This test reproduces the exact error:
"Failed to generate embeddings: Error code: 400 - "
"{'error': {'message': 'Requested 325998 tokens, "
...
class TestOversizedChunkReproduction:
    ...
    def test_config(self, temp_dir):
        ...
        return Config(
            args=fake_args,
            database={"provider": "duckdb", "path": str(db_path)},
            embedding={
                "provider": "openai",
    ...
    def services(self, test_config, temp_dir):
        ...
        return create_services(db_path=temp_dir / "test.db", config=test_config)
    ...
    async def test_analyze_chunking_behavior_with_large_files(
        ...
        for name, size, description in test_cases:
            ...
            if chunk_count > 0:
                ...
                if oversized:
                    print(
                        f"  ⚠️  Oversized chunks: {len(oversized)} (sizes: {oversized})"
                    )
                else:
