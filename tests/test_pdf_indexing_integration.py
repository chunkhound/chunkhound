...
def expected_content():
    ...
    assert markdown_path.exists(), (
        f"Expected content file not found at: {markdown_path}"
    )
...
class TestPDFIndexingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_pdf_complete_content_preservation(
        self, pdf_services, research_paper_pdf, expected_content
    ):
        ...
        assert result["status"] == "success", (
            f"PDF indexing failed: {result.get('error')}"
        )
    ...
    def _verify_chunk_structure(self, chunks: list[Chunk]):
        ...
        page_symbols = [
            chunk.symbol for chunk in chunks if chunk.symbol.startswith("page_")
        ]
        ...
        assert len(page_numbers) > 5, (
            f"Expected multiple pages, got: {sorted(page_numbers)}"
        )
        ...
        assert total_chars > 10000, (
            f"Total content seems too short: {total_chars} chars"
        )

        print(f"✅ Total content: {total_chars} characters across {len(chunks)} chunks")

    async def _verify_content_completeness(
        self, chunks: list[Chunk], expected_content: str, search: SearchService
    ):
        ...
        assert len(missing_sections) == 0, (
            f"Missing required content: {missing_sections}"
        )
    ...
    async def _verify_search_functionality(self, search: SearchService):
        ...
        for pattern, description in search_tests:
            ...
            found = any(
                pattern.lower() in result.get("content", "").lower()
    ...
    async def test_pdf_metadata_preservation(self, pdf_services, research_paper_pdf):
        ...
        pdf_filename_found = any(
            "cast_paper.pdf" in str(chunk.file_path)
            for chunk in chunks
            if chunk.file_path
        )
        # If no file paths are set, that's acceptable for this test
        if any(chunk.file_path for chunk in chunks):
            assert pdf_filename_found, (
                "Expected PDF filename in at least one chunk's file path"
            )
        ...
        print(
            f"✅ Metadata preserved: line numbers "
            f"{min(line_numbers)}-{max(line_numbers)}"
        )
    ...
    async def test_pdf_multi_page_handling(self, pdf_services, research_paper_pdf):
        ...
        for page_num, page_chunks in page_info.items():
            ...
            assert total_content > 50, (
                f"Page {page_num} has insufficient content: {total_content} chars"
            )
    ...
    async def test_pdf_content_ordering(self, pdf_services, research_paper_pdf):
        ...
        for chunk in sorted_chunks:
            assert chunk.start_line >= prev_line, (
                "Chunks should be in line number order"
            )
    ...
    async def test_pdf_search_precision(self, pdf_services, research_paper_pdf):
        ...
        for pattern, description in precision_tests:
            ...
            assert len(results) > 0, (
                f"No precise results for {description}: '{pattern}'"
            )
