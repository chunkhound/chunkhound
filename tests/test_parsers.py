...
LANGUAGE_SAMPLES = {
    ...
    Language.SQL: (
        ...
        "RETURNS INT BEGIN RETURN 0; END;\n"
...
def create_large_array_content(language: Language, item_count: int = 150) -> str:
    ...
    if language == Language.TOML:
        # Create a large dependencies array similar to
        # the actual pyproject.toml that broke. Include
        # realistic package names, version constraints,
        dependencies = []
...
class TestParserValidation:
    ...
    def test_parser_can_parse_minimal_code(self, language):
        ...
        try:
            ...
            # Web-language parsers must produce at least
            # one chunk for their sample inputs
            _must_have_chunks = {
                Language.HTML,
                Language.CSS,
                Language.SCSS,
            ...
            # SetupError indicates critical parser init
            # failure (e.g., version incompatibility)
    ...
    def test_parser_initializes_tree_sitter_language(self, language):
        """Test that parsers can initialize tree-sitter
        Language objects without version conflicts.

        This test specifically targets the tree-sitter
        language initialization where version compatibility
        is checked. This was the missing piece that allowed
        incompatible versions to pass CI tests.
        """
        factory = get_parser_factory()
        ...
        try:
            ...
            # SetupError during parser creation indicates
        ...
        try:
            ...
            pytest.fail(
                f"CRITICAL: Tree-sitter version "
                f"incompatibility for "
                f"{language.value}: {e}"
            )
        except Exception as e:
            # Check if this is a version incompatibility
            if "Incompatible Language version" in str(e):
                ...
                pytest.fail(
                    f"Unexpected error initializing "
                    f"tree-sitter language for "
                    f"{language.value}: {e}"
                )
    ...
    def test_parser_handles_long_arrays(self, language, item_count):
        ...
        print(
            f"\nTesting {language.value} with "
            f"{item_count} array items "
            f"({total_lines} lines)"
        )
        ...
        try:
            ...
            for chunk in chunks:
                if chunk.start_line > chunk.end_line:
                    ...
                    invalid_chunks.append(
                        f"{chunk.symbol}: end_line > total_lines "
            ...
            if invalid_chunks:
                error_msg = (
                    f"Found {len(invalid_chunks)} chunks with "
                    f"invalid line ranges in "
                    f"{language.value}:\n"
                )
