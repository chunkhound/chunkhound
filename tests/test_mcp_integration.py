        ...
        if api_key and provider:
            ...
            if provider == "openai":
                ...
                embedding_provider = VoyageAIEmbeddingProvider(
                    api_key=api_key, model="voyage-3.5"
                )
            else:
            ...
            if embedding_provider:
                embedding_manager.register_provider(
                    embedding_provider, set_default=True
                )
...
""")
    ...
    async def test_mcp_regex_search_finds_modified_files(self, mcp_setup):
        ...
        initial_results = await execute_tool(
            ...
            services=services,
            embedding_manager=None,
            arguments={"pattern": "initial_function", "page_size": 10, "offset": 0},
        )
        assert len(initial_results.get("results", [])) > 0, (
            "Initial content should be found"
        )
...
""")
    ...
    async def test_mcp_realtime_service_actually_starts(self, mcp_setup):
        ...
        assert stats.get("observer_alive", False), (
            "Filesystem observer should be running"
        )
