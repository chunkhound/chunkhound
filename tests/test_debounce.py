...
class TestDebounce:
    ...
    async def test_pending_files_cleared_on_external_cleanup(self, service):
        """Early return path cleans up pending_files (fix for stale entry bug)."""
    ...
    async def test_scan_then_change_produces_two_queue_entries(self, service):
        ...
        scan goes straight to file_queue; the subsequent change event still
        enters (priority=="change" bypasses the pending_files guard) and creates
        a debounce task. Processing is idempotent so the double-queue is safe.
        """
