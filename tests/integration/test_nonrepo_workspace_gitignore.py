...
def test_nonrepo_root_gitignore_affects_only_nonrepo(tmp_path: Path):
    ...
    subprocess.run(
        ["git", "-C", str(repo), "config", "user.email", "ci@example.com"], check=True
    )
    ...
    # Enable overlay explicitly via CLI flag:
    # datasets/ should be pruned; repo file should remain
    e = os.environ.copy()
    e.update(
        {"CHUNKHOUND_INDEXING__DISCOVERY_BACKEND": "git", "CHUNKHOUND_NO_RICH": "1"}
    )
    p = subprocess.run(
        [
            "uv",
    ...
    (ws / ".chunkhound.json").write_text(
        __import__("json").dumps(cfg), encoding="utf-8"
    )
    files_disabled = _simulate(
        ws, env={"CHUNKHOUND_INDEXING__DISCOVERY_BACKEND": "git"}
    )
