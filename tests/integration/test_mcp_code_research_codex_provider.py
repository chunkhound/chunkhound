...
async def test_code_research_uses_codex_cli_for_synthesis(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        codex_mod.CodexCLIProvider, "_run_exec", _stub_run_exec, raising=True
    )
    ...
    monkeypatch.setattr(
        tools_mod, "deep_research_impl", _stub_deep_research_impl, raising=True
    )
    ...
    assert isinstance(result, str) and "SYNTH_OK" in result, (
...
@pytest.mark.asyncio
async def test_code_research_forwards_path_argument_to_impl(
