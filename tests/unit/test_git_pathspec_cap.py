...
def test_git_pathspec_cap_falls_back_to_subtree(
    ...
    includes = [f"**/*.{i:03d}x" for i in range(20)] + [
        f"**/name{i:03d}.cfg" for i in range(20)
    ]

    # Force a small CAP to trigger the fallback
    monkeypatch.setenv("CHUNKHOUND_INDEXING__GIT_PATHSPEC_CAP", "2")

    files, stats = list_repo_files_via_git(
        repo, repo, include_patterns=includes, config_excludes=[]
    )
