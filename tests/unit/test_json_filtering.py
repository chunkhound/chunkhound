"""
Tests for config file filtering
(JSON/YAML/TOML) based on size and exclude patterns.
"""
...
async def test_json_size_threshold_filtering(tmp_path, real_components):
    ...
    assert small_result["status"] in ["complete", "success"], (
        f"Small JSON should be processed: {small_result}"
    )
    ...
    assert large_result["status"] == "skipped", (
        f"Large JSON should be skipped: {large_result}"
    )
    assert large_result["reason"] == "large_config_file", (
        f"Should skip for size reason: {large_result}"
    )
    ...
    assert large_file["content_hash"] is None, (
        "Large JSON skip record must have no content_hash"
    )
...
async def test_json_exactly_at_threshold(tmp_path, real_components):
    ...
    assert actual_size <= target_size, (
        f"File size {actual_size} should be <= {target_size}"
    )
...
"""
    ...
    assert small_result["status"] in ["complete", "success"], (
        f"Small YAML should be processed: {small_result}"
    )
    ...
    assert large_result["status"] == "skipped", (
        f"Large YAML should be skipped: {large_result}"
    )
    assert large_result["reason"] == "large_config_file", (
        f"Should skip for size reason: {large_result}"
    )
    ...
    assert large_file["content_hash"] is None, (
        "Large YAML skip record must have no content_hash"
    )
...
"""
    ...
    assert small_result["status"] in ["complete", "success"], (
        f"Small TOML should be processed: {small_result}"
    )
    ...
    assert large_result["status"] == "skipped", (
        f"Large TOML should be skipped: {large_result}"
    )
    assert large_result["reason"] == "large_config_file", (
        f"Should skip for size reason: {large_result}"
    )
    ...
    assert large_file["content_hash"] is None, (
        "Large TOML skip record must have no content_hash"
    )
...
async def test_json_filtering_in_directory_processing(tmp_path, real_components):
    ...
    (project_dir / ".eslintrc.json").write_text(
        json.dumps({"extends": "standard", "rules": {}})
    )
    ...
    exclude_patterns = [
        p for p in config.exclude if p not in ["**/tmp/**", "**/temp/**"]
    ]

    result = await coordinator.process_directory(
        project_dir, patterns=patterns, exclude_patterns=exclude_patterns
    )
    ...
    if os.environ.get("CI") == "true":
        ...
        try:
            ...
            files_in_db = [
                result.get("file_path", result.get("path", ""))
    ...
    # This is intentional — skipped files are
    # persisted to avoid re-scanning on each run.
    assert cache_file is not None, "Large JSON should have a skip record in DB (>20KB)"
    assert cache_file["content_hash"] is None, (
        "Large JSON skip record must have no content_hash"
    )
    # Excluded-by-pattern files are never processed
    # at all, so no DB record is created.
