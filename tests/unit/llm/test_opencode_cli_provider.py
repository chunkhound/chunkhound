...
class TestOpenCodeCLIProvider:
    ...
    async def test_complete_success(self):
        ...
        with patch.object(self.provider, '_run_cli_command', new_callable=AsyncMock) as mock_run:
    ...
    async def test_complete_structured_missing_required_fields(self):
        ...
        with patch.object(self.provider, '_run_cli_command', new_callable=AsyncMock) as mock_run:
            ...
            with pytest.raises(RuntimeError, match="LLM structured completion failed.*Missing required fields: \\['value'\\]"):
    ...
    async def test_batch_complete(self):
        ...
        with patch.object(self.provider, 'complete', new_callable=AsyncMock) as mock_complete:
    ...
    async def test_run_cli_command_retry_on_failure(self):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_subprocess:
            ...
            mock_process_success.communicate.return_value = (mock_json_response.encode(), b"")
