"""Phase 5: Parallel parsing — parse_batch_callback with thread pool.

When ``parse_thread_pool_size > 1`` and ``parse_batch_callback`` is provided,
the Rust pipeline dispatches batch-parsing calls across a Rust thread pool.
Each batch call uses Python's ``ProcessPoolExecutor`` internally for true
CPU-bound parallelism (tree-sitter holds the GIL).
"""
...
class TestParallelParse:
    """Parallel parsing must produce byte-identical output to serial parsing."""

    @pytest.mark.asyncio
    async def test_parallel_parse_engages_batch_callback(self):
        """RED: parse_batch_callback is called when parse_thread_pool_size > 1.
        ...
        def batch_callback(paths, detect_embedded_sql=True):
            nonlocal call_count
            call_count += 1
            return [
                parse_file_callback(p, detect_embedded_sql=detect_embedded_sql)
                for p in paths
            ]
        ...
        assert call_count > 0, (
            f"parse_batch_callback was never called! "
            f"parse_thread_pool_size=4 should trigger batch dispatch."
        )

    @pytest.mark.asyncio
    async def test_parallel_parse_identical_output(self):
        """Parallel parse produces identical chunk+embedding output to serial."""
