            ...
            with database_cleanup_context():
                ...
                try:
                    ...
                    assert response_time < 5.0, (
                        f"Server took {response_time:.2f} seconds to respond (should be < 5s)"
                    )
        ...
        with windows_safe_tempdir() as temp_path:
            ...
            with database_cleanup_context():
                ...
                try:
                    ...
                    assert "content" in stats_result, (
                        f"No content in stats result: {stats_result}"
                    )
                    ...
                    assert "initial_scan" in stats_data, (
                        f"No initial_scan in stats: {stats_data}"
                    )

                    scan_info = stats_data["initial_scan"]
                    assert "is_scanning" in scan_info, (
                        f"No is_scanning field: {scan_info}"
                    )
                    assert "files_processed" in scan_info, (
                        f"No files_processed field: {scan_info}"
                    )
                    assert "chunks_created" in scan_info, (
                        f"No chunks_created field: {scan_info}"
                    )
                    assert "started_at" in scan_info, (
                        f"No started_at field: {scan_info}"
                    )
