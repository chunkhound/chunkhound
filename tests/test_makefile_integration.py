...
"""
    ...
    variable_chunks = [
        c
        for c in chunks
        if c.symbol and c.symbol in ["CC", "CXX", "CFLAGS", "CXXFLAGS"]
    ]
    ...
    for chunk in chunks:
        assert len(chunk.code) <= max_chunk_size * 1.2, (
            f"Chunk exceeds size limit: {len(chunk.code)} > {max_chunk_size}"
        )
    ...
    print(
        f"   📏 Avg chunk size: {sum(len(c.code) for c in chunks) / len(chunks):.0f} chars"
    )
