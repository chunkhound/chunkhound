...
def test_extract_points_of_interest_parses_numbered_and_bullets() -> None:
    ...
    points = autodoc_pipeline._extract_points_of_interest(text, max_points=10)
...
def test_extract_points_of_interest_dedupes_and_respects_limit() -> None:
    ...
    points = autodoc_pipeline._extract_points_of_interest(text, max_points=2)

    assert points == ["Repeat", "Unique"]


def test_derive_heading_from_point_strips_formatting() -> None:
    heading = autodoc_pipeline._derive_heading_from_point("**Heading**: details here")
...
def test_derive_heading_from_point_truncates_long_text() -> None:
    long_text = "A" * 120
    heading = autodoc_pipeline._derive_heading_from_point(long_text)
...
def test_slugify_heading_normalizes_text() -> None:
    slug = autodoc_pipeline._slugify_heading("Heading: v2.1 (beta)!")

    assert slug == "heading-v2-1-beta"


def test_slugify_heading_caps_length() -> None:
    slug = autodoc_pipeline._slugify_heading("A" * 200)

    assert len(slug) <= 60


def test_coverage_summary_lines_with_denominators() -> None:
    lines = autodoc_pipeline._coverage_summary_lines(
        referenced_files=5,
