...
async def test_gemini_2_5_low_thinking():
    ...
    print(f"\nPrompt: {prompt}")
    print(f"Model: {provider.model}")
    print(f"Thinking level: {provider._thinking_level} (converted to thinking_budget=0)")
...
async def main():
    ...
    try:
        results.append(("Basic Completion (Gemini 3)", await test_basic_completion_gemini_3()))
        results.append(("Gemini 3 High Thinking", await test_gemini_3_high_thinking()))
        results.append(("Gemini 2.5 Low Thinking", await test_gemini_2_5_low_thinking()))
