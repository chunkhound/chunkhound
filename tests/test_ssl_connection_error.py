...
def _generate_mock_embedding(text: str, dims: int = 1536) -> list[float]:
    """Generate deterministic embedding vector from text hash."""
    text_hash = hashlib.sha256(text.encode()).digest()
    vector = [(text_hash[i % len(text_hash)] / 255.0) * 2 - 1 for i in range(dims)]
...
async def test_httpx_ssl_verify_env_var_doesnt_work():
    ...
    try:
        ...
        assert len(embeddings) == 1
        print(
            "HTTPX_SSL_VERIFY=0 env var now works, OR SSL config was fixed"
        )
