...
"""
        ...
        found = await wait_for_regex_searchable(
            services, "existing_function", timeout=get_fs_event_timeout()
        )
...
"""
        ...
        found = await wait_for_regex_searchable(
            services,
            "newly_added_content_unique_string",
...
"""
        ...
        found = await wait_for_regex_searchable(
            services, "added_content_edit_qa", timeout=get_fs_event_timeout()
        )
...
"""
        ...
        found = await wait_for_regex_searchable(
            services, "MODIFIED_existing_content", timeout=get_fs_event_timeout()
        )
...
<style>
  h1 {
    color: blue;
        ...
        db_stats = await services.indexing_coordinator.get_stats()
        print(
            f"📊 Final: {db_stats.get('files', 0)} files, {db_stats.get('chunks', 0)} chunks"
        )
