...
""")
    ...
    try:
        embeddings_query = (
            "SELECT COUNT(*) FROM embeddings_1024"  # VoyageAI uses 1024 dimensions
        )
        embedding_result = db.execute_query(embeddings_query)
        embedding_count = (
            list(embedding_result[0].values())[0] if embedding_result else 0
        )
...
""")
    ...
    if "total_embeddings" in initial_stats and "total_embeddings" in final_stats:
        embeddings_removed = (
            initial_stats["total_embeddings"] - final_stats["total_embeddings"]
        )
...
""")
    ...
    tasks = []
    for test_file in test_files:
        task = services.indexing_coordinator.process_file(
            test_file, skip_embeddings=False
        )
        tasks.append(task)
