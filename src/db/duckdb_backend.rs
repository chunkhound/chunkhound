...
impl crate::db::DbBackend for DuckDbHnswBackend {
    ...
    fn write_batches_in_one_txn(
        ...
        // BEGIN once for all batches.  prepare_write() was already called for
        // each batch in auto-commit mode (preserving the FK-constraint and
        // nested-BEGIN invariants), so only the insert phase runs here.
        {
            let conn = self.conn_or_err()?;
            conn.execute_batch("BEGIN")?;
        }
        ...
        for batch in batches {
            let inner = {
                let conn = self
                    .conn
                    .as_ref()
                    .expect("conn is Some: open() and BEGIN succeeded");
                match Self::write_batch_inner(conn, batch) {
                    Ok(r) => r,
                    Err(e) => {
                        let _ = conn.execute_batch("ROLLBACK");
            ...
            let embeddings_written = {
                let conn = self
                    ...
                    .as_ref()
                    .expect("conn is Some: open() and BEGIN succeeded");
                match Self::insert_embeddings_txn(conn, batch, &inner.embedding_pairs) {
                    Ok(n) => n,
                    Err(e) => {
                        let _ = conn.execute_batch("ROLLBACK");
                        return Err(e);
            ...
            results.push(BatchResult {
                file_ids: inner.file_ids,
                chunks_written: inner.chunks_written,
                embeddings_written,
            });
        }

        // COMMIT the whole window.  Auto-checkpoint may fire here — that is
        // expected, but now it fires once per N batches instead of once per batch.
        {
            let conn = self
                .conn
                .as_ref()
            ...
            log::debug!(
                "[store]   commit+checkpoint {:.1}ms ({} batches)",
                t_commit.elapsed().as_secs_f64() * 1e3,
                batches.len(),
            );
        }
