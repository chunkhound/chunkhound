...
ChunkHound uses `pre-commit` hooks to catch Python lint and formatting issues before review. Install them once after cloning:

```bash
uv run python scripts/pre_commit.py install
```

If you already manage your own `pre-commit` hook, either chain `uv run python scripts/pre_commit.py run-staged` from it or rerun the installer with `--overwrite-hook`.

The installed hook runs Ruff on the staged `.py` and `.pyi` snapshot. If Ruff rewrites any staged files, the commit stops on purpose so you can review the staged diff and commit again. For partially staged files, the fixed content stays in the index while unrelated unstaged edits remain unstaged.
...
4. **Test thoroughly** - Always run smoke tests before commits:
   ...
   ```
   If the hook rewrites staged files, review the staged diff and commit again.
