...
- **Remote configuration fetch** — ChunkHound can now fetch a configuration envelope from an operator-controlled HTTP endpoint at the start of every invocation and apply an ordered list of rules to the on-disk global config, letting fleets manage embedding / LLM / research settings centrally instead of touching each machine. See [`/docs/configuration/#remote-configuration`](https://chunkhound.dev/docs/configuration/#remote-configuration).
  ...
  - Delta-only validation gate: post-rules config is snapshot-validated against the current command **and** every persistence-hazard command (`index`, `mcp`, `research`); if any new `ConfigErrorCode` appears that wasn't already present pre-rules, the fetch is discarded and nothing is written.
