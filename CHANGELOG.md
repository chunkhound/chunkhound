...
- **Claude Fable 5 and Claude Mythos 5 support**: the Anthropic provider recognizes `claude-fable-5` and `claude-mythos-5` (launched 2026-06-09) with adaptive-only extended thinking (an explicit `manual` request auto-resolves to adaptive), effort levels `low`/`medium`/`high`/`xhigh`/`max`, and the task-budgets beta. These models run adaptive thinking server-side even when ChunkHound's thinking config is off; the provider logs a warning so the billed thinking tokens aren't a surprise.
