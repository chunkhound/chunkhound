...
def main():
    ...
    if fixture_path.exists():
        ...
        print(
            f"  {'✅' if interface_chunks else '❌'}"
            f" Interfaces: {len(interface_chunks)}"
        )
        if interface_chunks:
        ...
        print(
            f"  {'✅' if with_visibility else '⚠️ '}"
            f" Visibility modifiers: {len(with_visibility)}"
        )
        if with_visibility:
            vis_types = {}
        ...
        print(
            f"  {'✅' if abstract_chunks else '⚠️ '}"
            f" Abstract modifier: {len(abstract_chunks)}"
        )
        ...
        print(
            f"  {'✅' if len(with_code) == len(chunks) else '❌'}"
            f" All chunks have code:"
            f" {len(with_code)}/{len(chunks)}"
        )
        ...
        print(
            f"  {'✅' if len(symbols) == len(unique_symbols) else '⚠️ '}"
            f" Unique symbols:"
            f" {len(unique_symbols)}/{len(symbols)}"
        )
